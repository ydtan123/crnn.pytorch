...
def predict_one_file(fname, model, transformer, converter, word_length, debug=False):
    ...
    if (debug):
        print("-----{}".format(fname))
    for g in groups:
        gt, pred = predict_one_group(model, img, transformer, converter, g, word_length, debug)
        gt_list.append(gt)
...
def predict_one_clip(model, image, transformer, converter):
...
#    word_len = 1
    ...
    for idx in range(0, len(group), word_len):
        ...
        r, p = predict_one_clip(model, Image.fromarray(image[ty:by+1, tx:bx+1]), transformer, converter)
...
"""        
    if (args.dir != ''):
        ...
        for f in images:
            ...
            r, p = predict_one_clip(model, image, transformer, converter)
            if (p != gt[f.name]):
